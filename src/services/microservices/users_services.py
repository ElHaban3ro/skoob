...
class UsersServices:
    ...
    def create_user_token(self, email: str) -> str:
        ...
        with Session(self.engine) as session:
            ...
            return jwt.encode(to_encode, self.JWT_SECRET_KEY, algorithm="HS256")
        
    def create_user_token_by_username(self, username: str) -> str:
        user = self.ensure_admin_from_username(username)
        if not user:
            return ""
        to_encode = {"sub": user.email}
        expire = datetime.datetime.now(datetime.UTC) + datetime.timedelta(minutes=30)
